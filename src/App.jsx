...
export default function App() {
  ...
  // --- LISTEN FOR LOGIN STATUS (WITH SPIES) ---
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        setCurrentUser(user);
        try {
          console.log("SPY: User logged in. UID is:", user.uid);
          
          const userDoc = await getDoc(doc(db, 'users', user.uid));
          
          if (userDoc.exists()) {
            console.log("SPY: Database record found! Here is the data:", userDoc.data());
            setUserRole(userDoc.data().role);
          } else {
            console.log("SPY: ERROR! No database record found for this user! Defaulting to buyer.");
            setUserRole('buyer'); 
          }
        } catch (error) {
          console.error("SPY: Error fetching user role from database:", error);
          setUserRole('buyer'); 
        }
      } else {
        setCurrentUser(null);
