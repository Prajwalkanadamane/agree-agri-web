...
export default function AddRequirementModal({ isOpen, onClose, onAdd }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    onAdd({ ...formData, buyerName: 'Local Fresh Mart (You)' }); 
    setFormData({ produceName: '', quantity: '', targetPrice: '', deadline: '', deliveryLocation: '', notes: '' });
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/60 backdrop-blur-sm">
      <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl relative">
        ...
        <div className="p-6 sm:p-8">
          <h2 className="text-2xl font-bold text-gray-900 mb-6">Post a Bulk Requirement</h2>
          
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1">What do you need?</label>
                <input required type="text" name="produceName" value={formData.produceName} onChange={handleChange} placeholder="e.g., Potatoes (Jyoti variety)" className="w-full border border-gray-300 rounded-lg p-2 focus:ring-2 focus:ring-green-500 outline-none" />
              </div>
              <div>
            ...
            <div>
              ...
              <textarea name="notes" value={formData.notes} onChange={handleChange} rows="2" placeholder="Specific quality constraints, packaging requirements, etc." className="w-full border border-gray-300 rounded-lg p-2 focus:ring-2 focus:ring-green-500 outline-none resize-none"></textarea>
            </div>
